...
GetAccPmtData::GetAccPmtData(Event *event, int *stat, float *ped, float *sigp, float *pp, float *sgpp, int pr_hts)
{
  ...
  pnum = 17001;   //  The Aergel packets are 17001, 17005
  packetsperevent = 2; //
...
int GetAccPmtData::get_all_data_at_once(int ip)
{

  int ch_no;
  // Note that ich is set to zero in get_next_packet when each new packet is opened
  // ip is set to -1 in initialization, so we have started a new event, therefore we need a new packet

  int status = get_next_packet(ip);
  if (status == 1) { return 1;}
  //  cout << "get_packet is passed..." << endl;
  //  fflush(stdout);

  //  int idata=8;  // packets have 8 header words
  //  int tac=0,adcval=0,adcpost=0;
  //  int rawtac=0,rawadcpre=0,rawadcpost=0;
  //  int arrayno=0;
  ...
  while (1)
    {
      if (ch_no >= 160){ break;}
      int tac = -999, adcpost = -999, adcpre = -999;
...
int GetAccPmtData::get_next_packet(int fem_id)
{
  ...
  //  cout << "fem ID from packet = " << fem_id <<" fem ID from packet number= " << fem_id << endl;
  if (!(p = evt->getPacket(fem_id)))
    { 
      return 1;
    }
  //  int dlen=p->getDataLength();
  // Read the packet data into an integer array of length wordsperpacket
  p->fillIntArray(packetdata, 1000, &wordsperpacket, "DATA");
  ...
  if ((trail_err + head_err) > PERMIT_ACC_PACKET_ERROR)
    {
...
int GetAccPmtData::get_pmt_data(int *pmtn, int *adcp, int *adcv, int *tacv
                                , float *nh, int *sect)
{
  int fem_id = (ich / 160)*4 + 17001;
  if (ich % 160 == 0)
    {
      int j;
      // Now, the software is remade to take all the data at once.
      //    cout << "Coming normally till here (Before Array initialize)"<< endl;
      for (j = 0;j < 160;j++)
        {
          rawadcval[j] = rawadcpval[j] = rawtacval[j] = -900;
        }
      //    cout << "Coming normally till here (array initialize)"<< endl;
      //      fflush(stdout);
      get_all_data_at_once(fem_id);
    }
  //  cout << "Data taken at " <<fem_id<<"  "<< ich << endl;
  int pmtnum = AccGeom::get_pmtnum(ich % 160, fem_id);

  //  cout << "Here comes get_pmt_data..." << endl;
  if (pmtnum < 0 || pmtnum > 319)
    {
      // Some screwup in decoding the pmt number, punt on this event
      cout << "pmtnum= " << pmtnum;
      cout << " Screwup in data decoding, skip rest of this event" << endl;
      return 1;
    }
  ...
  float nhit = 0;
  //  float z=0,phi=0;
  if ( (float)rawadcval[ich % 160] > (pedestal[pmtnum] + 1*sigped[pmtnum]) && rawadcval[ich % 160] < 600 )
    {
      AccGeom::get_pmt_coords(pmtnum, &sector);
      if (pep[pmtnum] <= 5) {pep[pmtnum] = 80;}
      nhit = ((float)rawadcval[ich % 160] - pedestal[pmtnum]) / (pep[pmtnum] - pedestal[pmtnum]);
      if (print_hits)
        {
          cout << "Hit in PMT " << pmtnum << " adcval " << rawadcval[ich % 160] << " ped "
	       << pedestal[pmtnum] << " sigped " << sigped[pmtnum] << " 1pep "
	       << pep[pmtnum] << " nhits " << nhit << endl;
        }
    }
...
int GetAccPmtData::get_run_number(long *runout)
{
  *runout = run;
...
int GetAccPmtData::get_date(int *dt, int *tm)
{
  *dt = edate;
  *tm = etime;
...
int GetAccPmtData::decode_data_1X06(int rawtac, int rawadcpre, int rawadcpost, int *tac, int *adcval, int *adcpost, int *channel_no)
{
  int adcpre;
  //
  ...
  //
  //  *tac=(rawtac^0x0fff)&0x03ff;      // Read the data in the first 10 bits
  // adcpre=(rawadcpre^0x0fff)&0x03ff;
  //*adcpost=(rawadcpost^0x0fff)&0x03ff;
